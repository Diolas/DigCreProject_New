...
def segment_midi_data(notes, notes_per_section=8):
    """Segment MIDI data into sections and calculate features."""
    sections = []
    ...
    for i, (note, velocity, timestamp) in enumerate(notes):
        ...
        if (i + 1) % notes_per_section == 0 or i == len(notes) - 1:
            ...
            # Append section data
            sections.append({
                'notes': current_section['notes'],  # Ensure notes are added to the section
                'pitch_range': pitch_range,
                'average_velocity': avg_velocity,
                'tempo': tempo,
                'rhythm_complexity': rhythm_complexity,
                'note_density': note_density,
                'dominant_octave': dominant_octave  # Add dominant octave here
            })
